...
def performDBA(series, n_iterations=10):
    ...
    for i in tqdm(range(0, n_iterations), desc="DBA iterations"):
...
def approximate_medoid_index(series, cost_mat, delta_mat):
    ...
    medoid_ind = -1
    best_ss = 1e20
    for index_candidate in tqdm(indices, desc="Medoid search", leave=True, position=0):
...
def main():
    ...
    for i in range(0, n_series):
        ...
        series_i = np.pad(main_profile_gen, (n_pad_left, padding_length - n_pad_left), mode='constant',
                          constant_values=0)
